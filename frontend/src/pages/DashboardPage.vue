      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-ocean-deep-600">{{ $t('dashboard.welcome') }}, {{ user?.full_name }}</p>
          </div>
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'bookings'">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-lg font-semibold text-ocean-deep-900">{{ $t('dashboard.recentBookings') }}</h3>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-ocean-deep-200">
                <thead>
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">ID</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.guest') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.room') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.dates') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.status') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.total') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-ocean-deep-100">
                  ...
                  <tr v-else v-for="booking in bookings" :key="booking.id" class="hover:bg-ocean-deep-50">
                    ...
                    <td class="px-4 py-4 text-sm text-ocean-deep-600">{{ formatDate(booking.check_in) }} - {{ formatDate(booking.check_out) }}</td>
                    <td class="px-4 py-4">
                      <span :class="getStatusClass(booking.status)">
                        {{ $t(`dashboard.status_${booking.status}`) }}
                      </span>
                    </td>
                    <td class="px-4 py-4 text-sm font-medium text-ocean-deep-900">${{ booking.total_amount }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="activeTab === 'rooms'">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-lg font-semibold text-ocean-deep-900">{{ $t('dashboard.roomManagement') }}</h3>
              <button class="btn-primary text-sm">{{ $t('dashboard.addRoom') }}</button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div v-for="room in rooms" :key="room.id" class="card hover:shadow-lg transition-shadow">
                <div class="flex justify-between items-start mb-3">
                  <div>
                    <h4 class="font-semibold text-ocean-deep-900">{{ $t('rooms.room') }} {{ room.room_number }}</h4>
                    <p class="text-sm text-ocean-deep-600">{{ $t(`rooms.${room.room_type}`) }}</p>
                  </div>
                  <span :class="room.status === 'available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'" class="px-2 py-1 rounded-full text-xs font-medium">
                ...
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="font-medium text-ocean-deep-900">${{ room.price_per_night }}/{{ $t('dashboard.night') }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-ocean-deep-600">{{ $t('dashboard.capacity') }}:</span>
                    <span class="font-medium text-ocean-deep-900">{{ room.capacity }} {{ $t('dashboard.guests') }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-ocean-deep-600">{{ $t('dashboard.rating') }}:</span>
                    <span class="font-medium text-ocean-deep-900">{{ room.rating || 'N/A' }} ⭐</span>
                  </div>
                </div>
          ...
          <div v-if="activeTab === 'users'">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-lg font-semibold text-ocean-deep-900">{{ $t('dashboard.userManagement') }}</h3>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-ocean-deep-200">
                <thead>
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.name') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.email') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.role') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.status') }}</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-ocean-deep-600 uppercase">{{ $t('dashboard.joined') }}</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-ocean-deep-100">
                  <tr v-for="user in users" :key="user.id" class="hover:bg-ocean-deep-50">
                    ...
                    <td class="px-4 py-4 text-sm text-ocean-deep-600">{{ formatDate(user.created_at) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="activeTab === 'analytics'">
            <h3 class="text-lg font-semibold text-ocean-deep-900 mb-4">{{ $t('dashboard.analytics') }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="card">
                <h4 class="font-semibold text-ocean-deep-900 mb-4">{{ $t('dashboard.revenueChart') }}</h4>
              ...
              <div class="card">
                <h4 class="font-semibold text-ocean-deep-900 mb-4">{{ $t('dashboard.occupancyChart') }}</h4>
                <div class="h-64 flex items-center justify-center bg-ocean-deep-50 rounded-lg">
                  <p class="text-ocean-deep-600">{{ $t('dashboard.chartPlaceholder') }}</p>
                </div>
              </div>
...
const stats = ref([
  { label: t('dashboard.totalBookings'), value: '0', change: '+0%', icon: '📅', bgColor: 'bg-blue-100' },
  { label: t('dashboard.totalRevenue'), value: '$0', change: '+0%', icon: '💰', bgColor: 'bg-green-100' },
  { label: t('dashboard.occupancyRate'), value: '0%', change: '+0%', icon: '🏨', bgColor: 'bg-purple-100' },
  { label: t('dashboard.totalGuests'), value: '0', change: '+0%', icon: '👥', bgColor: 'bg-orange-100' }
])
...
const fetchData = async () => {
  ...
  try {
    ...
    // Update stats
    stats.value[2].value = `${Math.round((rooms.value.filter((r: any) => r.status === 'occupied').length / rooms.value.length) * 100)}%`
